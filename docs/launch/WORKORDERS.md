...
**Naming note (flagged, not resolved here):** the Production Execution Order refers to "Gate B — The Gate" (first arrival/consent/orientation) and "Gate C — The Clearing" (resource retrieval/escalation), which does not match this document's existing Gate B ("The Clearing Drill") / Gate C ("The Spine") labels and C1-C10 breakdown. Per the Founder's explicit instruction to prefer the latest founder decision over stale launch prose, the next work (decomposing Gate B and Gate C into implementation-sized work orders) must reconcile this naming conflict rather than silently pick one — flagged here so it isn't lost, not resolved as part of C2.

**C2 implementation note (V1 Scope Lockdown):** Executed per the Founder's Production Execution Order, which supersedes this row's original narrower "scope the Bench" framing — a read-only inspection found that six of the seven named Bench cuts (Spread, Binder, Pocket, Rehearsal, Company mode, Workshop, generative Mark) don't exist anywhere in the codebase as built features, so there was nothing to feature-flag for them; only voice is real and built. The actual work covered voice plus the two other LAUNCH-001-refused, fully-built member domains (Academy, Pods): a single central config (`apps/api/src/config/v1-feature-scope.ts`, mirrored at `apps/web/lib/config/v1-feature-scope.ts`) with `voice`/`academy`/`pods` flags, all default `false`. Backend: `V1ScopeMiddleware`, registered once in `AppModule.configure()` ahead of every guard/controller, 404s any request under `/ai/voice`, `/academy`, or `/pods` while its flag is off — closed by construction for every current and future sub-controller in those trees, not by per-controller annotation. `INTERFACE_ALLOWED_ROUTES` (the AI Steward's `navigate_to_route` tool allow-list, shared by voice and text) drops `academy` while its flag is off, closing a client-side-state bypass a plain nav/route audit would have missed. Frontend: `primarySurfaces` (the single nav + routing-scaffold source of truth) filters out Academy/Pods; their `page.tsx` routes redirect to `/home` directly (closing the stale-link/direct-route bypass, not just the nav link); `VoiceOrchestrator` is only mounted when its flag is on; `StewardWorkspace`'s voice UI is flag-gated as defense in depth on top of the backend block. All gated code stays in the tree, fully recoverable by flipping one boolean — nothing was deleted. A genuine bug was caught during verification: `V1ScopeMiddleware` initially read `req.path`, which Express 5 resolves to `/` for any middleware mounted via `forRoutes('*')`, making the guard a silent no-op; switched to `req.originalUrl`, confirmed against a live e2e run. Verified end-to-end: new middleware unit tests (prefix boundary, query-string handling), a new `v1-scope.e2e.spec.ts` proving 404 through the real HTTP/guard stack for a real authenticated member while an in-scope route (`/health`) stays reachable, `interface-tools.spec.ts`, and frontend tests for the nav filter, both redirects, and the palette/surface-tracker no longer offering Academy. Academy's and Pods' own existing e2e suites (and the AI e2e suite, which uses an Academy course as an insights fixture and exercises voice directly) flip their flag on for their own run so they keep proving the underlying domains still work — restored off in `afterAll`. Full validation: `tsc --noEmit` and `eslint` clean on both apps; full `apps/api` suite 113/115 suites passing (the same pre-existing Voice Domain/Recommendations flakiness and one pre-existing AI-summarize 503 in `connected-experiences.e2e.spec.ts`, both reproduced identically against an unmodified baseline before this work order); full `apps/web` suite 100/100 suites, 518/518 tests passing.
...
| C2 | Scope the Bench to the launch five | Limit the conversation/orchestration domain's working features to: Plan of stones, Plain Reading, Call Card, Knot, the Seal. Feature-flag off Spread, Binder, Pocket, Rehearsal, Company mode, Workshop, generative Mark, and voice. Per the Production Execution Order, expanded to a full V1 Scope Lockdown: voice, Academy, and Pods (LAUNCH-001's other fully-built, explicitly-refused member domains) closed via one central flag config, at the nav, direct-route, API, and AI-tool-navigation levels — not merely hidden. | None | Engineering | Critical | **Done** | Only the launch-five are reachable; all cut features are confirmed inaccessible in the launch build. |
...
- Gate A: 7 work orders (added A4-PREP — Human Steward Verification Workflow tooling)

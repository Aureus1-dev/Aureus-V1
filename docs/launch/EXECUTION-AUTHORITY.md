...
|---|---|---|
| What are we launching, for whom, and what is explicitly cut? | `docs/launch/LAUNCH-001-First-Members.md` | **Launch scope.** The Founder-facing execution order: the 25-member pilot, the Gates (A–F), what ships, what is refused. |
| What work is tracked, in what order, with what acceptance criteria? | `docs/launch/WORKORDERS.md` | **Execution registry.** The work-order-by-work-order breakdown of every Gate LAUNCH-001 defines. |
| What is done right now, what's next, what's blocked? | `docs/launch/SCOREBOARD.md` | **Current status.** A living dashboard only — it does not decide anything; it reflects what WORKORDERS.md records. |
| What was true of the platform as of a specific past milestone? | `docs/releases/version-1-readiness.md` | **Historical readiness and technical evidence.** A frozen snapshot (see the notice at the top of that document) — not a live status source. |

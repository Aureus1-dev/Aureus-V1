...
6. This repository had (until this incident) **no Render configuration at all** — no `render.yaml`, unlike the explicit `railway.json` already committed for Railway (§3). The most likely real root cause is exactly the ambiguity `railway.json`'s own comment warns about (a pnpm monorepo with two Dockerfiles, requiring an explicit `dockerfilePath` and a repository-root build context) going unaddressed for Render, most plausibly via a dashboard-only Root Directory or build configuration that doesn't match what `apps/api/Dockerfile` requires. This could not be confirmed directly (no access to the Render dashboard or the live deployment from this environment) — the fix below closes the gap regardless of which specific misconfiguration it was.

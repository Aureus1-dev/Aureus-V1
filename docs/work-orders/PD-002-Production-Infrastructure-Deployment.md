...
All 17 named items shipped, plus one pre-existing infrastructure defect discovered and fixed by this domain's own CI verification (see §3).
...
**Pre-existing infrastructure defect found and fixed by this domain's own CI verification (item 3):** the API runtime image never copied `prisma.config.ts` (root-level) into the runner stage — only `./prisma` (schema + migrations). `prisma/schema.prisma`'s own `datasource db` block has no `url`; it comes entirely from `prisma.config.ts`'s `datasource.url`. This meant `npx prisma migrate deploy` (and `db seed`) — the exact commands the production runbook has instructed operators to run via `docker compose exec api ...` since PR-002 — would fail at runtime inside the container with "The datasource.url property is required in your Prisma config file," even though `prisma generate` at build time worked fine (it runs from a stage that still has the whole repo checked out). This had never been caught because no CI job had ever actually executed a Prisma CLI command inside the *built* container before this domain's new `docker` job did — confirmed by that job's first real run failing on exactly this, then passing once `apps/api/Dockerfile` was fixed to copy `prisma.config.ts` alongside `./prisma`. This is precisely the class of gap CI-verified builds exist to catch, caught on the very first real build.

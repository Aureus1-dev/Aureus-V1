...
### PF-001 — Product-first baseline

Deliver:

- product-first thesis;
- exact three-repository health register;
- execution order;
- README execution-authority correction;
- stale-scoreboard notice;
- corrected Realtime example defaults.

Acceptance:

- V1 CI and Docker verification pass;
- only documentation and example configuration change;
- no safeguard, product route, schema, or authority is weakened.

### PF-002 — Library current-main consumer release

Repository: Aureus-Library

Deliver:

- rebuild the useful Retrieval v2 work from PR #8 on current main;
- include all 31 current Knowledge Objects;
- preserve Retrieval v1;
- generate deterministic release identity and exact revisions;
- approve only the minimum public retrieval set needed by the product;
- replace stale V1/Foundry pins with explicit contract fixtures rather than pretending current code already consumes the Library;
- maintain fail-closed integrity and opaque access denial;
- update current-state documentation from generated truth.

Acceptance:

- all Library validation and conformance checks pass;
- source commit, corpus hash, policy hash, release hash, object count, and exact revisions agree;
- no object is promoted or made public by tooling alone;
- PR #8 is recorded as superseded, not silently copied.

### PF-003 — Cross-repository contract package

Repositories: all three, separate coordinated PRs.

Deliver versioned schemas for:

- service identity and tenant context;
- Foundry work request/result;
- Library exact read/search pointer;
- consent and data-classification envelope;
- outcome/feedback event;
- error taxonomy and fail-safe behavior;
- compatibility and stale-pin tests.

Acceptance:

- contract fixtures pass in all three repositories;
- no repository imports another repository's internal implementation;
- version mismatch fails closed;
- retrieved text cannot change authority, tools, consequence, tenant, or approval.

## Release 1 — Sellable web pilot

### PF-004 — Business tenant foundation

Extend the existing Organization domain rather than creating a competing company model.

Deliver:

- organization as explicit tenant boundary;
- tenant memberships and operational roles;
- business profile, public status, service area, hours, contact routes, and escalation target;
- tenant-scoped audit context;
- database constraints and deny-path tests;
- business-console shell and onboarding.

### PF-005 — Business knowledge workspace

Deliver:

- tenant-owned source records;
- approved/under-review/rejected lifecycle;
- provenance, source URL/reference, freshness interval, next-review date, and accountable reviewer;
- service, FAQ, policy, pricing-boundary, geography, qualification, and escalation knowledge types;
- safe import and manual entry;
- no claim of legal advice or truth from upload alone;
- Library-candidate export for knowledge that deserves institutional reuse.

### PF-006 — Public Ward conversation

Deliver:

- public tenant-specific URL/embed;
- no Aureus account required;
- tenant-isolated guest conversation;
- grounded answers from approved tenant knowledge;
- honest “I don't know” and human escalation;
- mobile-first accessible interface;
- abuse, rate, spend, and prompt-injection controls;
- no leakage of internal notes, other tenants, or member data.

### PF-007 — Consented lead and handoff

Deliver:

- explicit lead-capture consent;
- minimum contact and project fields;
- conversation attribution;
- qualification as transparent signals, not hidden persuasion;
- assignment and notification;
- human acceptance, contact, close, loss, and reason states;
- customer-visible handoff confirmation;
- retention and deletion behavior.

### PF-008 — Business console

Deliver:

- inbox/pipeline;
- conversation and grounded-source view;
- assigned owner and next action;
- handoff state and outcome;
- business-hours and fallback configuration;
- knowledge freshness queue;
- provider/spend/health summary;
- export and correction paths.

### PF-009 — Kitchen-and-bath vertical pack

Deliver:

- governed thesis for the vertical;
- intake for project type, rooms, scope, location, desired timing, ownership/decision status, budget range only when appropriate, design needs, and preferred contact;
- service and exclusion templates;
- estimation boundary: no fabricated quote;
- photo/file intake with consent and retention;
- scheduling/handoff template;
- evaluation set based on realistic remodeler questions and failure cases.

### PF-010 — Pilot offer and operability

Deliver:

- one founding plan after a 30-day pilot;
- tenant setup checklist;
- support and incident flow;
- usage and cost accounting;
- data-processing and privacy disclosures;
- accessibility review;
- backup, restore, monitoring, alerting, and rollback;
- exact deployment manifest across all three repositories.

## Release 2 — Full Business V1 continuity
...
- inbound call disclosure and consent;
- after-hours and missed-call capture;
- callback queue;
- SMS link with opaque continuation token;
- same conversation and tenant memory on the web;
- handoff to the business's sales team;
- call recording only where lawful, configured, and consented;
- no claim that a bot is a human.

### PF-012 — Founder walkthrough and first client

The Founder completes, on mobile and desktop:
...
10. failure-mode and provider-outage drills.

Only after automated gates and this walkthrough pass at the same exact deployment manifest may the first external business begin its pilot.

## Explicitly deferred

The following are not first-pilot blockers:

- succession and permanent governance design;
- member elections and institutional voting tiers;
- broad constitutional consolidation;
- autonomous consequential action;
- full CRM replacement;
- autonomous sales closing;
- video studio and B-roll management;
- 3D room design/rendering;
- national multi-vertical launch;
- public member launch beyond the existing controlled pathway.

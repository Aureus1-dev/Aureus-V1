...
const makeOrg = (o: Partial<Organization> = {}): Organization => ({
  ...
  shortDescription: 'Free legal help',
...
describe('OrganizationsService', () => {
  ...
  describe('update', () => {
    ...
    it('allows the OWNER representative to update', async () => {
      mockRepo.findById.mockResolvedValue(makeOrg());
      mockMemberRepo.findByOrgAndUser.mockResolvedValue(
    ...
    it('forbids a non-member', async () => {
      ...
      await expect(service.update('org-uuid', {}, OTHER_MEMBER)).rejects.toThrow(
        ForbiddenException,
      );
    });
  ...
  describe('verification workflow', () => {
    it('submitForReview moves DRAFT to PENDING_REVIEW for an ADMIN representative', async () => {
      ...
      mockRepo.update.mockResolvedValue(
        makeOrg({ verificationStatus: VerificationStatus.PENDING_REVIEW }),
      );
    ...
    it('submitForReview throws ConflictException when not DRAFT', async () => {
      mockRepo.findById.mockResolvedValue(
        makeOrg({ verificationStatus: VerificationStatus.VERIFIED }),
      );
      mockMemberRepo.findByOrgAndUser.mockResolvedValue(makeMembership());

      await expect(service.submitForReview('org-uuid', ADMIN_REP)).rejects.toThrow(
        ConflictException,
      );
    });
    ...
    it('verify throws ConflictException when not PENDING_REVIEW', async () => {
      mockRepo.findById.mockResolvedValue(
        makeOrg({ verificationStatus: VerificationStatus.DRAFT }),
      );
    ...
    it('reject moves PENDING_REVIEW to REJECTED with a reason', async () => {
      ...
      mockRepo.update.mockResolvedValue(
        makeOrg({
          ...
          rejectionReason: 'Could not verify',
        }),
      );
